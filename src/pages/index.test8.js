...
const RetroResearchTerminal = () => {
      ...
      <div className="space-y-6">
        {ideas.map((idea, ideaIndex) => (
          <div key={ideaIndex} className="mb-6 border border-black p-4">
            ...
            {(idea.literatureNoveltyScore !== undefined || typeof checkLiteratureNovelty === 'function') && (
              <div className="mt-4 text-sm">
                <div className="flex items-center gap-4">
                  ...
                  {idea.literatureNoveltyScore !== undefined ? (
                    ...
                    <button 
                      onClick={async () => handleNoveltyCheck(idea, ideaIndex)}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Make all initial API requests in parallel
      ...
      // In handleSubmit, after getting analysisData:
      if (analysisData.novelSentences) {
        console.log('Setting novel sentences:', analysisData.novelSentences);
        setNovelSentences(analysisData.novelSentences);
      } else {
        // Fallback to using scoredText if novelSentences isn't available
        const sentences = analysisData.scoredText?.map(item => ({
          text: item.text,
          score: item.score
        })) || [];
        console.log('Setting novel sentences from scoredText:', sentences);
      ...
      // Update novel sentences from analysis data
if (analysisData.scoredText) {
  setNovelSentences(analysisData.scoredText.map(item => ({
    text: item.text,
    score: item.score
  })));
}
      // Generate research ideas if we have valid analysis data
      if (analysisData.scoredText?.[0]) {
        const topScoredText = analysisData.scoredText[0];
        console.log('Generating ideas for top scored text:', topScoredText);
        ...
        <div style={{ 
          ...
          <div className="pt-16 p-8">
            ...
            <div className="border border-black bg-white mb-8 max-w-6xl mx-auto shadow-sm">
              ...
              <div className="p-4">
              {currentPage === 0 ? (
// In the paper text rendering section:
